...
function str(str) {
    return function (input, start) {
        return new Promise(function (resolve, reject) {
            if (str === input.substr(start, str.length)) resolve([str, str.length + start]);else reject("<str " + str + "> failed at " + start);
        });
    };
}
...
function re(pattern) {
    ...
    return function (input, start) {
        return new Promise(function (resolve, reject) {
            ...
            if (result) resolve([result[0], result[0].length + start]);else reject("<re " + pattern.source + "> failed at " + start);
        });
    };
}
